...
def tfidf_filter(myverbalizer, cc_logits, class_labels):
    ...
    sim_mat = cosine_similarity(label_words_cc_logits.cpu().numpy(), first_label_logits.cpu().numpy()).reshape(
        *orgshape[:-1], first_label_logits.shape[0])
    ...
    for class_id in range(len(myverbalizer.label_words)):
        ...
        for in_class_id in range(max_lbw_num_pclass):
            if myverbalizer.label_words_mask[class_id, in_class_id] > 0:
                ...
                idf_score = 1 / (
                            np.linalg.norm(idf_score_source, ord=norm_ord) / np.power((class_num - 1), 1 / norm_ord))
